...
import { fetchVehicles, getVehicleSlice } from '../../Slices/SaleSlice'
function VehicleSale() {
    const dispatch = useDispatch()
    const vehicles = useSelector(getVehicleSlice)
    console.log("vehicles",vehicles);

    useEffect(()=>{
        dispatch(fetchVehicles(getVehicleSlice))
    },[dispatch])
  return (
    <div>
        <Container>
       ...
       <Row>
        ...
        {vehicles.map((value) => {
              ...
              return (
                <Link className='car-link' to={`/single-vehicle/${value.mtr_slug}`}>
